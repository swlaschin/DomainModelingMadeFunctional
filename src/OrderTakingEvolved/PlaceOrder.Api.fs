...
let internal getStandardPrices() : GetProductPrice =
    fun productCode ->
...
let internal getPricingFunction :GetPricingFunction =
    PricingModule.getPricingFunction getStandardPrices  getPromotionPrices

let internal calculateShippingCost =
    Implementation.calculateShippingCost

let internal createOrderAcknowledgmentLetter : CreateOrderAcknowledgmentLetter =
    fun pricedOrder ->
        let letterTest = HtmlString "some text"
        letterTest

let internal sendOrderAcknowledgment : SendOrderAcknowledgment =
